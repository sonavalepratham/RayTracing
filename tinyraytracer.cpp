...
struct Material { // body material, contains color
    ...
    Vec3f albedo; // array of 3 elements. 1st diffused albedo, 2nd specular albedo, 3rd reflection
...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, size_t depth=0) {
    ...
    for (size_t i=0; i<lights.size(); i++) {
        ...
        float light_distance = (lights[i].position - point).norm(); // light to point distance.
        Vec3f shadow_orig = light_dir*N < 0 ? point - N*1e-3 : point + N*1e-3; // checking if the point lies in the shadow of the lights[i]
        // above we are adding very small value to point to prevent hitting the current sphere itself.
        Vec3f shadow_pt, shadow_N; // shadow_pt - vector from light src to sphere on which light blows. shadow_N normal on that surface.
...
int main() {
    ...
    Material     mirror(Vec3f(0.0, 10.0, 0.8), Vec3f(1.0, 1.0, 1.0), 1425.); // difused multiplier will be zero for mirror
